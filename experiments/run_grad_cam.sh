...
# 单样本 Grad-CAM 可视化（grad_cam.py）：每个模型随机抽 num_samples 个分子，
# 每个分子展示预测概率最高的 topk_labels 个标签热力图
# 用法: bash run_grad_cam.sh [num_samples] [topk_labels]
...
for exp in $EXPS; do
    ...
    echo "===== 开始: ${exp} (num_samples=${NUM_SAMPLES}, topk_labels=${TOPK_LABELS}) ====="
    python grad_cam.py --exp "$exp" --num_samples "$NUM_SAMPLES" --topk_labels "$TOPK_LABELS"
done

echo "全部跑完，结果分别在 results/{exp}/gradcam/{file_id}.png"
